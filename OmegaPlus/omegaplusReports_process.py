...
from omegaplusReports_functions import process_OmegaReport, normalize_events, bar_chart_plot, distance_scatterplot, distance_histogram, distance_lineplot
...
directory = './'
file_name = 'omegaplusReports_parameters.ini'
...
# Subdirectory containing the OmegaPlus Reports (e.g. /home/directory).
...
# Filter the dataset to exclude all entries with lower omega value than the threshold
...
if __name__ == "__main__":
    ...
    """
    ------------------------------
    Process OmegaPlus Report files
    ------------------------------
    """ 
    all_results = []
    for dir_name in os.listdir(directory):
        ...
        if os.path.isdir(dir_path) and dir_name.startswith("simulation_"):
            result_dict = process_OmegaReport(infection_rate, position, dir_path, reports_dir)
            if result_dict:
