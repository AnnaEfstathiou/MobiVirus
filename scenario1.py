...
# while tt <= 10000:
    ...
    if sum(coords_t[:, 5]==1) < s_n * sum(coords_t[:, 5]==2): 
        ...
        print("Break because Normal<Super") 
         
        break

    if sum(coords_t[:, 5]==1) + sum(coords_t[:, 5]==2) > m * n:
        print("Break because of m criteria!")
        break
    
    if (ss + ns) > 100:
        print("Break because of 100 infections!")
        print ("Super", ss)
        print("Normal", ns)
        break
    ...
    if t_s >= rec_t+t_im and coords_t[:,2].any() == 1: # If the event time is bigger than the minimum recovery time and if there are infected people,
        ...
        coords_t[normal_indx, 6] = 1  # hange the susceptibility label back to 1
    ...
    ## In the other case, where s1 is bigger than p_m, infection will be the event happening ##
        ...
        for j in range(n): 
            if ipi[j] != 0 and coords_t[j,2] == 0 and coords_t[j,6] == 1: 
                ...
                print (f"MUTATION LABEL OF THE INFECTED: {coords_t[j,5]}")

                print("tOTAL INFECTED:", sum(coords_t[:, 2]==1))
