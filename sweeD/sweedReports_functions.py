...
def process_SweeDReport(infection_rate, position, directory, reports_dir):   
    '''
    Processes SweeD_Report files in a specified directory, identifying the genomic position with the highest 
    composite likelihood. Computes the distance between this peak and a reference position, returning results
    as a dictionary.
    ...
    - reports_dir (str): Subdirectory within the root directory containing SweeD_Report files.

    Returns:
    - dict: A dictionary keyed by filenames, with values as lists containing:
        [infection_rate, directory, event_number, max_likelihood, max_position, distance_to_reference]
    '''
    ...
    # Collect files starting with "SweeD_Report"
    report_files = [f for f in os.listdir(full_reports_dir) if f.startswith("SweeD_Report")]

    if not report_files:
        logging.warning(f"No SweeD_Report files found in '{full_reports_dir}'.")
        return {}
    ...
    for file in report_files:
        try:
            event_match = re.search(r'\.(\d+)_', file)
            if not event_match:
            ...
            # Find the line where headers start
            header_line_idx = next((i for i, line in enumerate(lines) if line.strip().startswith("Position")), None)
            if header_line_idx is None:
                logging.warning(f"Skipping '{file}': No header line found.")
                continue

            column_names = ["Position", "Likelihood", "Alpha", "StartPos", "EndPos"]
            ...
            if 'Likelihood' not in data.columns:
                logging.warning(f"Skipping '{file}': 'Likelihood' column missing. Columns found: {data.columns.tolist()}")
                continue

            idx_max = data['Likelihood'].idxmax()
            max_likelihood = data.at[idx_max, 'Likelihood']
            max_position = round(data.at[idx_max, 'Position'])
            distance = abs(max_position - position)

            results_dict[file] = [
                infection_rate, directory, event_number,
                max_likelihood, max_position, distance
            ]
