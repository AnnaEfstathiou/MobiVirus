...
def LD(sequences, strain_type, sample):
    """
    Compute linkage disequilibrium (LD) from sampled sequence data using libsequence.
    Args:
        sequences (str): Path or identifier for the sequence dataset to be analyzed.
        strain_type (str): Type of strain to sample from. Options:
                        - "mix_strains": Use both normal and super strains.
                        - "ss_strains": Use only super strains.
                        - "ns_strains": Use only normal strains.
        sample (int): Number of individuals to sample from the population.
    Returns:
        pd.DataFrame:
            A dataframe of LD statistics (r² values) computed between all pairwise SNPs
            in the simulated data, using libsequence's `ld` method.
    """
...
if __name__ == "__main__":
    ...
    ld_df.to_csv(f"ld_{suffix}.csv", index=False) 
